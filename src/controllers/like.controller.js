...
const toggleCommentLike = asyncHandler(async (req, res) => {
  ...
  const commentlikedby = req.user._id;
  if (!commentlikedby || !isValidObjectId(commentlikedby)) {
    throw new ApiError(400, "Invalid user id");
  }
  const likedchecked = await Like.findOne({
    comment: commentId,
  ...
  if (!likedchecked) {
    ...
    await Like.findByIdAndDelete(likedchecked._id);
    return res
      .status(200)
